...
export const appName = process.env.APP_NAME || 'EducationCA™';
      ...
      if (tenant) {
        ...
        try {
          ...
          if (res) {
            ...
            const usedStorage = response?.usedStorage
              ? response.usedStorage + size
              : size;
...
export const updateMailCount = async (extUserId, plan, monthchange) => {
  ...
  try {
    ...
    console.log(
      'Error updating EmailCount in contracts_Users: ' + error.message,
    );
  }
};
...
export function formatWidgetOptions(type, options) {
  ...
  const status = options?.required === true ? 'required' : 'optional';
  ...
  switch (type) {
    ...
    case 'textbox':
      return {
        ...
        validation: {
          type: 'regex',
          pattern: options?.regularexpression || '/^[a-zA-Z0-9s]+$/',
        },
        fontColor: fontColor,
        fontSize: fontSize,
      };
...
export const useLocal = process.env.USE_LOCAL
  ? process.env.USE_LOCAL.toLowerCase()
  : 'false';
export const smtpsecure =
  process.env.SMTP_PORT && process.env.SMTP_PORT !== '465' ? false : true;
export const smtpenable =
  process.env.SMTP_ENABLE && process.env.SMTP_ENABLE.toLowerCase() === 'true'
    ? true
    : false;
