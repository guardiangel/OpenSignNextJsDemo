...
import { planCredits } from '@/utils/UtilsForOpenSign';

export function defineBuyaddonusers() {
  Parse.Cloud.define('buyaddonusers', async (request) => {
        ...
        if (resExtUser) {
          ...
          if (resSub) {
            ...
            if (res.data.access_token) {
              ...
              const price =
                _resSub?.SubscriptionDetails?.data?.subscription?.plan?.price;
              const plan_code =
                _resSub?.SubscriptionDetails?.data?.subscription?.plan
                  ?.plan_code;
              const addonsArr =
                _resSub?.SubscriptionDetails?.data?.subscription?.addons || [];
              let addon = 0;
              ...
              const quantity =
                parseInt(users) +
                (typeof addon === 'number' ? addon : parseInt(addon));
              ...
              const updatedSubscription = await axios.put(
                'https://www.zohoapis.in/billing/v1/subscriptions/' +
                  subscriptionId,
                data,
                {
                  headers: {
                    ...
                    'X-com-zoho-subscriptions-organizationid':
                      process.env.ZOHO_BILLING_ORG_ID,
                  },
                },
              );
              ...
              const resupdateSub = await updateSub.save(null, {
                useMasterKey: true,
              });
