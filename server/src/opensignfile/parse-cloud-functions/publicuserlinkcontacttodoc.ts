...
export function definePublicuserlinkcontacttodoc() {
  Parse.Cloud.define('publicuserlinkcontacttodoc', async (req) => {
        ...
        if (resSub) {
          ...
          if (totalcredits > 0) {
            ...
            if (index !== -1) {
              ...
              const existContact = await contactCls.first({
                useMasterKey: true,
              });
              if (existContact) {
                ...
                const extUser = await extUserQuery.first({
                  useMasterKey: true,
                });
                if (extUser) {
                  ...
                  try {
                    ...
                    const userRes = await userQuery.first({
                      useMasterKey: true,
                    });
                    if (userRes) {
                      const contact = {
...
const deductcount = async (_resSub) => {
  try {
    ...
    console.log(
      'Err in deductcount in PublicUserLinkContacttoDoc cloud function',
      err,
    );
  }
};
...
const createDocumentFromTemplate = async (template, existContact, index) => {
  try {
    if (template) {
      ...
      object.set(
        'FileAdapterId',
        template?.FileAdapterId || fileAdapterId_common,
      );
...
const sendMailToAllSigners = async (docId) => {
  try {
    ...
    if (res) {
      ...
      try {
        const getSubscriptionDetails = await Parse.Cloud.run(
          'getsubscriptions',
          {
        ...
        if (getSubscriptionDetails) {
          ...
          const subscription_json = JSON.parse(
            JSON.stringify(getSubscriptionDetails),
          );
          const billingDate =
            subscription_json?.result?.Next_billing_date &&
            subscription_json?.result?.Next_billing_date?.iso;
          const isSubscribed = billingDate
        ...
        console.log('error in get partners_Tenant class details', e);
      }
    }
  } catch (e) {
    console.log('error in sendMailToAllSigners function', e);
  }
};
