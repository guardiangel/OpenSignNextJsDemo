...
import { parseJwt } from '@/utils/UtilsForOpenSign';

export function defineSavecontact() {
  Parse.Cloud.define('savecontact', async (request) => {
    ...
    if (request.user) {
      ...
      const decoded = jwt.verify(jwttoken, appRes?.get('token')) as any;
