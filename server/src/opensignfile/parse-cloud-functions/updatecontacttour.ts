...
export function defineUpdatecontacttour() {
  Parse.Cloud.define('updatecontacttour', async (request) => {
    ...
    try {
      ...
      if (contactRes) {
        ...
        const tourStatus =
          _contactRes?.TourStatus?.length > 0 ? _contactRes.TourStatus : [];
        let updatedTourStatus = [];
        if (tourStatus.length > 0) {
          updatedTourStatus = [...tourStatus];
          const requestSignIndex = tourStatus.findIndex(
            (obj) =>
              obj['requestSign'] === false || obj['requestSign'] === true,
          );
