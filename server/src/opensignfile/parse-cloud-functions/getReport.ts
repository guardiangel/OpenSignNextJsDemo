...
import reportJson from '@/opensignfile/assistfunctions/reportsJson';
dotenv.config();

export function defineGetReport() {
  Parse.Cloud.define('getReport', async (request) => {
    ...
    try {
      ...
      if (userId) {
        ...
        const clsName = json?.reportClass
          ? json.reportClass
          : 'contracts_Document';
        if (json) {
          ...
          if (reportId == '6TeaPr321t') {
            ...
            if (extUser) {
              ...
              if (_extUser?.TeamIds && _extUser.TeamIds?.length > 0) {
                let teamArr = [];
                _extUser?.TeamIds?.forEach(
                  (x) => (teamArr = [...teamArr, ...x.Ancestors]),
                );
