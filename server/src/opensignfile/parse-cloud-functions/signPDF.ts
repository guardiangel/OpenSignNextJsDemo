...
import GenerateCertificate from '@/opensignfile/assistfunctions/GenerateCertificate';
import uploadFileToAzure from '@/opensignfile/assistfunctions/uploadFileToAzure';

const APPID = process.env.XParseApplicationId;
const masterKEY = process.env.XParseMasterKey;
const eSignName = process.env.eSignName;
const eSigncontact = process.env.eSigncontact;

export function defineSignPDF(configService: ConfigService) {
  Parse.Cloud.define('signPdf', async (req) => {
    try {
      ...
      const fileAdapterId = _resDoc?.FileAdapterId || fileAdapterId_common;
      let adapterConfig = {};
      if (fileAdapterId) {
      ...
      if (reqUserId) {
        ...
        const _contractUser = _resDoc.Signers.find(
      ...
      if (req.params.pdfFile) {
        ...
        const auditTrail = updateAuditTrail.filter(
          (x) => x.Activity === 'Signed',
        );
        ...
        const docName = _resDoc?.Name?.replace(
          /[^a-zA-Z0-9._-]/g,
          '_',
        )?.toLowerCase();
        ...
        if (isCompleted) {
          const signersName = _resDoc.Signers?.map(
            (x) => x.Name + ' <' + x.Email + '>',
          );
          if (signersName && signersName.length > 0) {
            ...
            pdflibAddPlaceholder({
              pdfDoc: pdfDoc,
              reason:
                'Digitally signed by EducationCA for ' +
                signersName?.join(', '),
              location: 'n/a',
              name: eSignName,
        ...
        if (data && data.imageUrl) {
          ...
          const updatedDoc: any = await updateDoc(
          ...
          sendDoctoWebhook(_resDoc, data.imageUrl, 'signed', signUser, null);
        ...
        const error = new Error('Pdf file not present!') as any;
        error.code = 400; // Set the error code (e.g., 400 for bad request)
        throw error;
      }
    } catch (err:any) {
...
async function sendNotifyMail(doc, signUser, mailProvider) {
  try {
    ...
    if (remaingsign > 1 && doc?.NotifyOnSignatures) {
      ...
      const mailLogo = AVATAR_URL;
      const viewDocUrl = `${process.env.OpenSignFrontEndAddress}/recipientSignPdf/${doc.objectId}`;
      ...
      const body =
        ...
        `</div></div><div><p>This is an automated email from EducationCA™. For any queries regarding this email, please contact the sender ${creatorEmail} directly. If you think this email is inappropriate or spam, you may file a complaint with EducationCA™ <a href=https://EducationCA.com/#/about target=_blank>here</a>.</p></div></div></body></html>`;
      ...
      await axios.post(cloudServerUrl + '/functions/sendmailv3', params, {
        headers: {
...
async function sendMailsaveCertifcate(
  ...
  pdflibAddPlaceholder({
    ...
    reason: 'Digitally signed by EducationCA.',
    location: 'n/a',
    name: eSignName,
    contactInfo: eSigncontact,
    signatureLength: 15000,
  ...
  await axios.put(
    cloudServerUrl + '/classes/contracts_Document/' + doc.objectId,
    body,
    {
      headers: {
        'Content-Type': 'application/json',
...
async function sendCompletedMail(obj) {
  ...
  let body =
    ...
    `<b>"${pdfName}"</b>` +
    '. Kindly download the document from the attachment.</p></div> </div><div><p>This is an automated email from EducationCA™. For any queries regarding this email, please contact the sender ' +
    sender.Email +
    ' directly. If you think this email is inappropriate or spam, you may file a complaint with EducationCA™ <a href=EducationCA.com target=_blank>here</a>.</p></div></div></body></html>';
...
async function uploadFile(pdfName, filepath, fileAdapterId, configService) {
  try {
    ...
    if (fileAdapterId) {
      ...
      try {
        ...
        return { imageUrl: presignedUrl?.blobUrl };
      } catch (err:any) {
...
async function updateDoc(docId, url, userId, ipAddress, data, className, sign) {
  try {
    ...
    const signedRes = await axios.put(
