...
export default async function GenerateCertificate(docDetails) {
  ...
  const auditTrail =
    docDetails?.Signers?.length > 0
      ? docDetails.AuditTrail.map((x) => {
          const data = docDetails.Signers.find(
            (y) => y.objectId === x.UserPtr.objectId,
          );
          return {
            ...data,
  ...
  let yPosition5 = 395;
  let yPosition6 = 360;
  auditTrail.slice(0, 3).forEach(async (xItem, i) => {
    const embedPng = xItem.Signature
      ? await pdfDoc.embedPng(xItem.Signature)
      : '';
    ...
    page.drawText(`${new Date(xItem.ViewedOn).toUTCString()}`, {
      x: half + 112,
      y: yPosition2,
      size: timeText,
      font: timesRomanFont,
    ...
    page.drawText(`${new Date(xItem.SignedOn).toUTCString()}`, {
      x: half + 108,
      y: yPosition3 + 5,
      size: timeText,
      font: timesRomanFont,
