...
import {
  ...
  getFileName, toDataUrl
} from "@/newComponents/opensigncomponents/constant/Methods";
import { SaveFileSize } from "@/newComponents/opensigncomponents/constant/saveFileSize";
import { fileAdapterId_common } from "@/newComponents/opensigncomponents/constant/Utils";
import { formJson } from "@/newComponents/opensigncomponents/json/FormJson";
...
import DropboxChooser from "@/newComponents/opensigncomponents/shared/fields/DropboxChoose";
...
import OpenSignPageNotFound from "./OpenSignPageNotFound";
...
function OpenSignForm() {
  ...
  if (config) {
    ...
    return <OpenSignPageNotFound prefix={"Form"} />;
  }
}
...
const Forms = (props) => {
            ...
            if (isImage) {
              ...
              if (isEnableSubscription) {
                try {
                  ...
                  alert(
                    "We are currently experiencing some issues with processing DOCX files. Please upload the PDF version or contact us on support@EducationCa.com"
                  );
                }
              }
            }
  ...
  /**
   * should be deprecated after modification, keep it here temporarily GuiquanSun 20250228
   * @param base64 
   * @param filename 
   * @param fileAdapterId 
   * @returns 
 ...
 */
  const getFile = (file: any) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.addEventListener('load', () => {
            // Ensure that reader.result is not null and is of type ArrayBuffer
            if (reader.result instanceof ArrayBuffer) {
                // This will result in a byte array
                const bytes = Array.from(new Uint8Array(reader.result));

                resolve({
                    // Full name - option to rename (add original name to attribute list?)
                    documentSize: file.size,
                    // Base64 encoded Document
                    documentEncodedContent: btoa(
  ...
  const handleFileUpload = async (file:any) => {
      ...
      if (fileAdapterId) {
        ...
        if (fileRes.url) {
          setFileUpload(fileRes?.url?.blobUrl);
          const tenantId = localStorage.getItem("TenantId");

          const title = generateTitleFromFilename(file.name);
          setFormData((obj) => ({ ...obj, Name: title }));
          SaveFileSize(size, fileRes.url?.blobUrl, tenantId);
        }
        setfileload(false);
        setpercentage(0);
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (fileupload) {
      ...
      try {
        ...
        object.set("FileAdapterId", extUserData?.TenantId?.ActiveFileAdapter?extUserData?.TenantId?.ActiveFileAdapter:fileAdapterId_common);
        ...
        if (res) {
          ...
          setFileUpload("");
          setpercentage(0);
          router.push(`/${props?.redirectRoute}/${res.id}`);
          setIsAlert((obj) => ({
            ...obj,
            type: "success",
  ...
  const handleCancel = () => {
    router.push("/dashboard/35KBoSgoAK");
  };
  const handlePasswordSubmit = async (e) => {
    window.alert("when clicking next button");
    return;
  };
