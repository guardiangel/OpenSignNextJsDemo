import logo from "@/newComponents/opensigncomponents/assets/images/logo.png";
    ...
    try {
      ...
      if (!docDetails.error) {
        if (sendmail === "false") {
          ...
          router.push(`/load/recipientSignPdf/${docId}/${contactId}`);
        }
        return true;
      } else {
        console.log("GuestLogin.tsx docDetails.error:", docDetails?.error);
        setDocDetailError(docDetails?.error);
        return false;
      }
    } catch (err:any) {
              ...
              <>
                {!EnterOTP ? (
                  <div className="w-full md:w-[50%] text-base-content">
                    ...
                    {docDetailError && <h1 className="text-2xl md:text-[30px] text-red-500">{docDetailError},Please make sure the document does still exist!</h1>}
