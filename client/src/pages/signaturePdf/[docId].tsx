...
import EmailComponent from '@/newComponents/opensigncomponents/pdf/EmailComponent';
...
import Signedby from '@/newComponents/opensigncomponents/pdf/Signedby';
import SignPad from '@/newComponents/opensigncomponents/pdf/SignPad';
import TextFontSetting from '@/newComponents/opensigncomponents/pdf/TextFontSetting';
import VerifyEmail from '@/newComponents/opensigncomponents/pdf/VerifyEmail';
import WidgetComponent from '@/newComponents/opensigncomponents/pdf/WidgetComponent';
import DownloadPdfZip from '@/newComponents/opensigncomponents/primitives/DownloadPdfZip';
...
import PdfChatbot from '@/newComponents/PdfChatBot/PdfChat';
...
function SignYourSelf() {
    ...
    const [containerWH, setContainerWH] = useState<ContainerSize>({
        width: 0,
        height: 0,
    });
    ...
    const senderUser =
        localStorage.getItem(`Parse/${XParseApplicationId}/currentUser`)! &&
        localStorage.getItem(`Parse/${XParseApplicationId}/currentUser`)!;
    ...
    const getDocumentDetails = async (showComplete) => {
        ...
        try {
            ...
            if (documentData && documentData.length > 0) {
                //
                const pineconeNamespaceResult: any =
                    await getCreateFileVectorization(documentData[0].URL, null);
                ...
                console.log(
                    `createFileVectorization:`,
                    pineconeNamespaceResult?.data?.result
                );

                setPineconeNamespace(pineconeNamespaceResult?.data?.result);
            ...
            if (contractUsersRes === 'Error: Something went wrong!') {
                ...
                const contractContactBook = await contactBook(
                    jsonSender.objectId
                );
                if (contractContactBook && contractContactBook.length > 0) {
                    ...
                    if (tourstatuss && tourstatuss.length > 0 && !isCompleted) {
                        ...
                        if (
                            checkTourRecipients &&
                            checkTourRecipients.length > 0
                        ) {
                            setCheckTourStatus(
                                checkTourRecipients[0].signyourself
                            );
                        }
                    } else {
                        setCheckTourStatus(true);
    ...
    const addPositionOfSignature = (item, monitor) => {
        ...
        if (item === 'onclick') {
            ...
            const getWidth = widgetTypeExist
                ? calculateInitialWidthHeight(widgetValue, dragTypeValue)
                      .getWidth
            ...
            dropObj = {
                ...
                isStamp:
                    (dragTypeValue === 'stamp' || dragTypeValue === 'image') &&
                    true,
                key: key,
                type: dragTypeValue,
    ...
    const handleVerifyBtn = async () => {
        ...
        console.log(
            '[docId] handleVerifyBtn currentEmail:',
                    ...
                    const addSign = getPosData.map((url) => {
                        if (url.key === dragKey) {
                            return {
                                ...url,
                                xPosition:
                                    dragElement.x / (containerScale * scale),
                                yPosition:
                                    dragElement.y / (containerScale * scale),
                            };
                        }
                        return url;
                    });
    ...
    const handleDeleteSign = (key) => {
        ...
        let filterData = xyPostion[index].pos.filter(
            (data) => data.key !== key
        );
        ...
        if (isDontShow) {
            ...
            if (tourStatus.length > 0) {
                ...
                const signyourselfIndex = tourStatus.findIndex(
                    (obj) =>
                        obj['signyourself'] === false ||
                        obj['signyourself'] === true
                );
                if (signyourselfIndex !== -1) {
                    updatedTourStatus[signyourselfIndex] = {
                        signyourself: true,
                    };
                } else {
            ...
            await axios
                .put(
                    `${OpenSignServerURL}/classes/contracts${contractName}/${signerUserId}`,
                    {
