...
import { isEnableSubscription, themeColor } from "@/newComponents/opensigncomponents/constant/const";
import { SaveFileSize } from "@/newComponents/opensigncomponents/constant/saveFileSize";
import {
  addWidgetOptions,
  addZIndex,
  AVATAR_URL,
...
import DropdownWidgetOption from "@/newComponents/opensigncomponents/pdf/DropdownWidgetOption";
import { EmailBody } from "@/newComponents/opensigncomponents/pdf/EmailBody";
import Header from "@/newComponents/opensigncomponents/pdf/PdfHeader";
import PdfZoom from "@/newComponents/opensigncomponents/pdf/PdfZoom";
import PlaceholderCopy from "@/newComponents/opensigncomponents/pdf/PlaceholderCopy";
...
import RotateAlert from "@/newComponents/opensigncomponents/pdf/RotateAlert";
import SignerListPlace from "@/newComponents/opensigncomponents/pdf/SignerListPlace";
import WidgetComponent from "@/newComponents/opensigncomponents/pdf/WidgetComponent";
import WidgetNameModal from "@/newComponents/opensigncomponents/pdf/WidgetNameModal";
import Alert from "@/newComponents/opensigncomponents/primitives/Alert";
import LottieWithLoader from "@/newComponents/opensigncomponents/primitives/DotLottieReact";
import LinkUserModal from "@/newComponents/opensigncomponents/primitives/LinkUserModal";
...
import QuotaCard from "@/newComponents/opensigncomponents/primitives/QuotaCard";
import TourContentWithBtn from "@/newComponents/opensigncomponents/primitives/TourContentWithBtn";
import Upgrade from "@/newComponents/opensigncomponents/primitives/Upgrade";
...
function PlaceHolderSign() {
  ...
  const [signersdata, setSignersData] = useState<any[]>([]);
  const [signerPos, setSignerPos] = useState<any[]>([]);
  const [isSelectListId, setIsSelectId] = useState<any>();
  const [isSendAlert, setIsSendAlert] = useState<any>({});
  ...
  const [containerWH, setContainerWH] = useState<any>();

  const router = useRouter();
  const { placeHolderSignDocId,state }:any = router.query; 
  ...
  const documentId = placeHolderSignDocId;

  useEffect(() => {
     //Set up the loading status when refreshing the page.
     setIsLoading({ isLoad: true, message: t("loading-mssg") }); // Start loading
    if (documentId) {
      getDocumentDetails();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [documentId]);//must set documentId here if need to refresh the page
  ...
  const saveDocumentDetails = async () => {
    ...
    try {
      ...
      await axios
        .put(
          `${OpenSignServerURL}/classes/contracts_Document/${documentId}`,
          data,
          {
            headers: {
    ...
    return shareLinkList.map((data, ind) => {
      return (
        <div
          ...
          key={ind}
        >
          {copied && <Alert type="success" className={""}>{t("copied")}</Alert>}
  ...
  const sendEmailToSigners = async () => {
    ...
    setIsSendAlert({});
    let sendMail;
    // Search database to get ExpiryDate
    const documentData = await contractDocument(documentId,null);
    let expireDate = new Date();
    if (documentData && documentData.length > 0) {
      expireDate = documentData[0] && documentData[0]?.ExpiryDate?.iso;
    }
    ...
    for (let i = 0; i < signerMail.length; i++) {
      try {
        ...
        let signPdf = `${hostUrl}/login/${encodeBase64}`;
        const openSignUrl = "https://vassutech.com";
        ...
        let params = {
          ...
          html: isCustomize
            ...
            : "<html><head><meta http-equiv='Content-Type' content='text/html; charset=UTF-8' /> </head>   <body> <div style='background-color: #f5f5f5; padding: 20px'=> <div   style=' box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 12px;background: white;padding-bottom: 20px;'> <div style='padding:10px 10px 0 10px'><img src=" +
              ...
              senderEmail +
              " directly.If you think this email is inappropriate or spam, you may file a complaint with VassuTech™   <a href= " +
              openSignUrl +
  ...
  const closeTour = async () => {
    ...
    if (isDontShow) {
      ...
      try {
        await axios.put(
          `${OpenSignServerURL}/classes/contracts_Users/${signerUserId}`,
  ...
  const handleRecipientSign = () => {
    router.push(`/recipientSignPdf/${documentId}/${currentId}`);
  };
  ...
  return (
    <>
      <Title title={state?.title ? state.title : "New Document"} drive={""}/>
