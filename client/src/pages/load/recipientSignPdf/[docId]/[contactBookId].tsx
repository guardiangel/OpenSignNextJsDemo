...
import DefaultSignature from "@/newComponents/opensigncomponents/pdf/DefaultSignature";
...
import SignerListComponent from "@/newComponents/opensigncomponents/pdf/SignerListComponent";
...
function PdfRequestFiles(props) {
          ...
          if (!isEnableOTP) {
            try {
              const resContact = await axios.post(
                `${OpenSignServerURL}/functions/getcontact`,
                {
                  contactId: currUserId
                },
                {
              ...
              if (!pdfBytes?.error) {
                ...
                if (resSign && resSign.status === "success") {
                  ...
                  const user = usermail?.Email
                    ...
                    console.log(`[contactBookId].tsx sendmail:${sendmail}, sendInOrder:${sendInOrder}`);
  ...
  const declineDoc = async (reason) => {
    ...
    await axios
      .post(`${OpenSignServerURL}/functions/declinedoc`, params, {
        headers: {
          "Content-Type": "application/json",
          "X-Parse-Application-Id": XParseApplicationId,
  ...
  const closeRequestSignTour = async () => {
    ...
    if (isDontShow) {
      ...
      if (!isEnableOTP) {
        try {
          await axios.post(
            `${OpenSignServerURL}/functions/updatecontacttour`,
    ...
    if (!emailRegex.test(contact.email)) {
      ...
      try {
        ...
        const userRes = await axios.post(
          `${OpenSignServerURL}/functions/publicuserlinkcontacttodoc`,
          params,
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const SendOtp = async () => {
    try {
      ...
      const Otp = await axios.post(
        `${OpenSignServerURL}/functions/SendOTPMailV1`,
        params,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const VerifyOTP = async (e) => {
    ...
    setLoading(true);
    const serverUrl =
      OpenSignServerURL && OpenSignServerURL;
    const parseId =
      XParseApplicationId && XParseApplicationId;
    if (otp) {
      // setLoading(true);
      try {
